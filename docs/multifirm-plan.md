...
3. **ゲスト用ライブラリ**: A+B の3秒保持と、KEY.B を押したままの起動でホストへ戻る。
...
M5StopWatch-MultiFirm/
  ...
  host/multifirm_host.cpp       # ESP-IDF コンポーネントだけがビルド
  tools/multifirm.py
  tools/multifirm.ps1
  tools/layout.py
...
- ゲストは PlatformIO の `lib_deps`、ホストは ESP-IDF コンポーネントとして取り込む。
- UserDemo は `repos.json` と `fetch_repos.py` で `components/multifirm` に取り込む。
...
ゲストはヘッダオンリーとし、ホスト実装は PlatformIO のゲスト用ソース探索から外す。ESP-IDF の CMake でのみホスト実装を登録する。
ヘッダオンリーでも Arduino・ESP-IDF 双方のコンパイル確認は行う。共有状態は inline 関数内の static 等で複数翻訳単位からの利用を考慮する。
`multifirm_layout.h` と `return_gesture.h` は IDF ヘッダに依存させず、C++17 のネイティブコンパイラで tests を実行できるようにする。テスト用 CMake/CTest と実行コマンドを用意する。

## 4. レイアウト

ホスト4 MiB、ゲスト各1,984 KiB、共有 NVS 64 KiB、専用メタデータ16 KiB の配置を v1 の固定レイアウトとする。
...
アプリスロット内のメタデータ予約は行わない。0x9f0000〜0xa00000 の64 KiB と、0xe10000〜0x1000000 の1,984 KiB は未割り当てで残す。
...
| `install` | イメージ・容量・実機の表を検証し、指定スロットと対応するメタデータセクタだけを更新する。NVS・otadata・他スロットは書かない。 |
...
| `initial` | 全体バックアップを保存・検証し、新配置のブートローダ・表・ホストを書き、NVS・otadata・全ゲスト領域・メタデータを初期化する。PHY データは必要な構成なら対応データを書き、不要な構成では消去する。storage・coredump は書かない。 |
...
**IDF 環境を推奨し、Arduino コアにはパッチしない。** 確認した Arduino コアの `initArduino()` は `setup()` より前に、`NO_FREE_PAGES` または `NEW_VERSION_FOUND` で NVS 全体を消す。Arduino ゲストを使用する場合は、他アプリの設定や BLE ボンド情報を含む共有 NVS 全体が消える可能性を許容する。専用の `multifirm_meta` と storage はこの NVS 消去の対象外である。
...
- A+B をタッチなしで3000ms保持すると、1回の保持につき1回だけ発火する。ボタン解除・タッチによるキャンセルと再受付、時刻の wraparound をテストする。
...
5. ゲストのビルド対象にホスト実装が含まれないことと、ヘッダの複数翻訳単位での使用を確認する。
...
3. `app_name.py` 等は他の用途がないことを確認してから削除する。
...
- [ ] Arduino の全消去経路が残ることを記録し、試験用 NVS の明示的な全消去後も、アプリが既定設定で復旧でき、メタデータ・storage が保持されることを確認する。
...
ホスト4 MiBの新配置、専用メタデータ、Arduino コア無変更、ローカル guard、付加ダイジェストの再利用は確定事項。追加のレイアウト変更や標準ビルド統一を断念する変更は、結果と選択肢を整理して判断する。
