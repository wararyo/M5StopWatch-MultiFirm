...
- リリースタグ `v1.0.0` を打つ。タグ付けはユーザーが行う。
  タグの前に `library.json` の `version` と `tools/multifirm.py` の `TOOL_VERSION`
  （どちらも `0.1.0`）を合わせる必要がある。
- タグ後に3ゲストの `lib_deps` を `#a5d9529` から `#v1.0.0` へ切り替える。
  コードは同一なので再ビルド確認だけでよく、実機への再インストールは不要。
